    ...
    def load_data(self, item, config, transfroms=None, fixed=False):
        ...
        if config.transform.use_patch:
            ...
            if fixed:
                rand_x = 500
                rand_y = 500
            input = input_with_ratio[str(ratio)[:3]][image_id][:, rand_x : rand_x + patch_size, rand_y : rand_y + patch_size, :]
    ...
    def train(self):
        ...
        for epoch in range(traincfg.epochs):
            for _, data in enumerate(dataloader):
                ...
                if completed_step % setup.log_every_n_step == 0 or completed_step == total_step:
    ...
    def eval(self, is_training=False, global_step=0):
        ...
        with torch.no_grad():
            ...
            for _, data in enumerate(tqdm(dataloader, desc=eval_type)):
                ...
                input, labels = self.load_data(data[0], datacfg, transfroms=None, fixed=True)
            ...
            if is_training and self.writer is not None:
                self.writer.add_scalar(
                    psnr_meter.name, psnr_meter.mean, global_step
                )
                self.writer.add_scalar(
                    ssim_meter.name, ssim_meter.mean, global_step
                )
